...
namespace rabint\attachment\helpers;
...
class FormatValidation
{
    ...
    private static function getMaxSizeDescription($rules)
    {
        ...
        return Yii::t('rabint', 'Max. file size') . ': ' . $maxSize . ' ';
    }
    ...
    private static function getExtensionDescription($rules)
    {
        ...
        return Yii::t('rabint', 'File types') . ': ' . $extensions . ' ';
    }
    ...
    private static function getImageSizeDescription($rules)
    {
        ...
        if (self::isImageWithStrictSize($rules)) {
            $text .= Yii::t('rabint', 'Image size') . ': ' . $maxWidth . 'x' . $maxHeight . 'px;';
        } elseif (self::isImageWithMinAndMaxSize($rules)) {
            $text .= Yii::t('rabint', 'Min. size of image') . ': ' . $minWidth . 'x' . $minHeight . 'px;';
            $text .= Yii::t('rabint', 'Max. size of image') . ': ' . $maxWidth . 'x' . $maxHeight . 'px;';
    ...
    private static function prepareImageSizeDescription($rules, $exclude = [])
    {
        ...
        foreach ($rules as $rule => $value) {
            switch ($rule) {
                case 'minWidth':
                    $text .= Yii::t('rabint', 'Min. width') . ' ' . $value . 'px;';
                    break;
                case 'minHeight':
                    $text .= Yii::t('rabint', 'Min. height') . ' ' . $value . 'px;';
                    break;
                case 'maxWidth':
                    $text .= Yii::t('rabint', 'Max. width') . ' ' . $value . 'px;';
                    break;
                case 'maxHeight':
                    $text .= Yii::t('rabint', 'Max. height') . ' ' . $value . 'px;';
                    break;
            }
        }
